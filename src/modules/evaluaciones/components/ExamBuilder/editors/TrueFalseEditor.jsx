...
export default function TrueFalseEditor({ pregunta, onChange, onRemove, indice }) {
  ...
  const selectorRespuesta = (valor) => {
    onChange({
      ...pregunta,
      respuestaCorrecta: valor,
      respuestaCorrectaLabel: valor ? 'Verdadero' : 'Falso',
    });
  };
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <textarea
            ...
            placeholder="Enunciado (afirmación a evaluar)..."
            rows={3}
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <button
          type="button"
          onClick={() => selectorRespuesta(true)}
          className={`p-3 rounded-xl text-center font-black transition ${
        ...
        <button
          type="button"
          onClick={() => selectorRespuesta(false)}
          className={`p-3 rounded-xl text-center font-black transition ${
      ...
      <div className="mt-4">
        <label className="block text-[9px] uppercase font-black text-slate-400 tracking-widest italic mb-1 ml-1">
          Justificación de la respuesta correcta (opcional)
        </label>
        <textarea
          value={pregunta.justificacionCorrecta || ''}
          onChange={(e) => onChange({ ...pregunta, justificacionCorrecta: e.target.value })}
          placeholder="Explica por qué la afirmación es verdadera/falsa..."
          rows={2}
